...
void lcd_rotatingBanner(char dispText[]){
    ...
    do {
                ...
                // Initialize the array with blank chars for padding
                for(first_i = 0; first_i < 20; first_i++) {

                  int q;
                  for(q = 0; q < 20 - first_i; q++){
                      text_to_display[q] = ' ';
                  }
              if(first_i != 0) {
                int w, e;
                w = 20 - first_i;
                // Start copying in the user's input string
                for(e = 0; e < first_i; e++){
                  text_to_display[w] = dispText[e];
                  w++;
                }

                int s;
                for(s = 20; s >= strlen(dispText); s--){
                    text_to_display[s] = dispText[s+1];
                }

                lcd_printf(text_to_display);
                  }
                  else {
                lcd_printf(text_to_display);
                  }
                ...
                // Display the 'middle' chunks of the string
                int second_i;
                for(second_i = 20; second_i < strlen(dispText); second_i++) {
                  int q;
                  for(q = 0; q < 20; q++) {
                     text_to_display[q] = text_to_display[q + 1];
                  }
                  text_to_display[19] = dispText[second_i];

                  lcd_printf(text_to_display);
                  timer_waitMillis(300);
                }
                // Shift the string around
                int third_i;
                   for(third_i = 0; third_i < 20; third_i++) {
                       int q;
                     for(q = 0; q < 20 - third_i; q++) {
                   text_to_display[q] = text_to_display[q + 1];
                     }
                     if(third_i != 0) {
                   int w;
                   for(w = 20 - third_i; w < 20; w++) {
                     text_to_display[w] = ' ';
                   }
                     }
                     lcd_printf(text_to_display);
